...
ssh ${user}@${host} "[ -d ${evo_version_path} ] && echo 'exists' || mkdir -p  ${evo_version_path}"
...
for image in $initial_images; do
    ...
    scp -o StrictHostKeyChecking=no -i ${private_key} out/target/product/$device/$image ${user}@${host}:/${build_date_path}
    echo " "
done
